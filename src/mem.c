...
addr_t alloc_mem(uint32_t size, struct pcb_t *proc)
{
    ...
    * */
	...
	if (cr_num_pages >= num_pages && NUM_PAGES * PAGE_SIZE - proc->bp >= num_pages * PAGE_SIZE)
		mem_avail = 1;
	else
		ret_mem = 0;
	...
	if (mem_avail)
	{
		...
		for (int i = 0, j = 0, k = 0; i < NUM_PAGES; i++)
		{
			if (_mem_stat[i].proc == 0)
			{
				...
				int booler = 0;
				/*Nếu đã có first_index trong seg_table*/
				...
				/*Nếu chưa có first_index trong seg_table -> tạo bảng mới*/
				if (booler == 0)
				{
...
int free_mem(addr_t address, struct pcb_t *proc)
{
    ...
    *      processes.  */
	...
	if (translate(address, &physical_addr, proc))
	{
		int next = -2;
		int i = 0, j = 0;
		/*Xoá cái đầu tiên ứng với địa chỉ address -> tìm ra cái next*/
		for (; i < NUM_PAGES; i++)
		{
		...
		/*Ứng với cái next vừa tìm được, ta xoá những cái tiếp theo*/
		next = i;
		while (next != -1)
		{
			...
			/*----------------*/
			/*Xoá trong segment table và page tables*/
			...
			for (int n = 0; n < proc->seg_table->size; n++)
			{
				if (proc->seg_table->table[n].v_index == first_lv)
				{
					for (int m = 0; m < proc->seg_table->table[n].pages->size; m++)
					{
						...
						{
							/*----------------*/
							/*Dồn page table lại, xong xoá cái cuối cùng*/
							int k = 0;
